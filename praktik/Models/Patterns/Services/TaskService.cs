...
namespace praktik.Models.Patterns
{
    ...
    public class TaskService
    {
        ...
        public bool ApplyChecklistDecision(Task task, bool isCompleted, int completedStatusId, int incompleteStatusId, int userId, string comment, out string errorMessage, int postponeDays = 1)
        {
            ...
            if (!TryValidateChecklistDecision(task, isCompleted, completedStatusId, incompleteStatusId, userId, comment, postponeDays, out errorMessage))
            {
                return false;
            }
            ...
            try
            {
                ...
                if (!isCompleted)
                {
                    PostponeTask(task, postponeDays);
                }

                db.UpdateTask(task);
                db.AddTaskReport(task.TaskId, userId, BuildChecklistReportText(isCompleted, comment));
                return true;
            }
            catch (Exception ex)
            {
        ...
        private bool TryValidateChecklistDecision(Task task, bool isCompleted, int completedStatusId, int incompleteStatusId, int userId, string comment, int postponeDays, out string errorMessage)
        {
            errorMessage = null;
        ...
        private static void PostponeTask(Task task, int postponeDays)
        {
            task.StartDate = task.StartDate.Date.AddDays(postponeDays);
            task.EndDate = task.EndDate.Date.AddDays(postponeDays);
        }

        private static string BuildChecklistReportText(bool isCompleted, string comment)
        {
            ...
            if (isCompleted)
            {
                return string.IsNullOrWhiteSpace(normalizedComment)
                    ...
                    : $"Задача выполнена: {normalizedComment}";
            }

            return $"Задача не выполнена и перенесена: {normalizedComment}";
        }
